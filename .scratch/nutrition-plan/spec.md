...
- `Plan { id, startDate, endDate: string | null, dayTypes: DayType[], supplements: PlanSupplement[] }`
  - `DayType { id: string, name: string, weekdays: Weekday[], references: PlanReference[], slots: PlanSlot[] }` — a generic list, not fixed `dayTypeA`/`dayTypeB` fields. `Weekday` uses the existing native `Date` convention (Sunday `0` through Saturday `6`). `references` holds non-trackable menu text such as "Al despertar — Agua: 300 ml".
  - `PlanSlot { id: string, name: string, dishes: PlanDish[] }` — `name` corresponds to the menu's own slot labels (Desayuno, Comida, Cena, Media tarde, etc.); "Al despertar"/"Medio día" water-only entries are represented as reference text, not as trackable slots (see story 10). Stable identifiers let slot completions link to the saved Plan without relying on array positions.
  ...
  - `PlanIngredient { name: string, quantityText: string, grams: number | null, unit: "g" | "ml" | null }` — `quantityText` preserves the original text (e.g. "⅓ taza"); `grams` is the parsed numeric equivalent used for shopping-list aggregation, `unit` preserves whether that equivalent is grams or milliliters, and both are null when the source menu doesn't state one. Ingredient preparation details remain in `name`; aggregation normalizes known preparation/variety variants such as `Pepino, pelado`, `Pepino pelado`, and `Pepino, con cáscara` to one shopping item.
...
- `AppState` gains: `plans: Plan[]`, `planSlotCompletions: PlanSlotCompletion[]` (stable id, date + planId + dayTypeId + slotId + linked `Meal.id`), `planSupplementLogs: PlanSupplementLog[]` (stable id, date + planId + supplement name), and `shoppingListState` (per-ingredient bought/not-bought flags, manually cleared — not scoped per plan version, since it tracks a physical shopping trip in progress). Ticket 01 keeps the existing legacy `supplementName`/`supplementDose` settings and `supplementLogs` because current scoring and UI still use them; issue 05 handles their migration.
...
**Shopping list:**
- Computed (not stored precomputed) by a pure function: for the active plan and a given week, sum each quantified ingredient amount across every slot/dish occurrence on days where the Plan is active, multiplied by how many times each day-type occurs in the week. Ingredients without a numeric equivalent are listed with their original `quantityText`, ungrouped. The aggregation processes explicit `PlanIngredient` records only; it does not use a hardcoded exclusion list. Reference-only text and a prescribed dish with no known recipe ingredients (such as the menu's Agua de jamaica) remain visible but contribute nothing to the list.
- `shoppingListState` (bought/not-bought per normalized ingredient name and unit) persists independently of the computed list and is only cleared by an explicit user action (no automatic weekly reset).
